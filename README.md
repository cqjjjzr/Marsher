...
若想删除列表中的某个提问，将其拖动到主窗体中“拖放在此以删除”处即可。同时你也可以**按键盘上的`Del`键**删除选定的提问。
...
在任意列表中选中一组棉花糖，向外拖动到资源管理器或桌面中，即可导出选定的棉花糖。

![图16](./.github/Figure16.png)

可将导出的文件分发给其他人（例如字幕组成员，或由字幕组成员整理后发给本人）。接收数据的人不必登录帐号或执行拉取操作，仅需将需要导入的`.marsher`文件拖动到列表区域即可。

![图17](./.github/Figure17.png)

![图18](./.github/Figure18.png)

可以选择导入到已有列表或新的列表。导入时会自动将所有的提问加入“所有收到的提问”列表。
